...
def find_rand_destination_reg():
    ...
    if previousIntegerDestinations and prob < writeAfterWrite:
        suitableNums = [
            n for n in previousIntegerDestinations if n not in cv.unusedRegs +
            cv.branchRegisters]
        if suitableNums:
    ...
    # Write after read
        ...
        suitableNums = [
            n for n in previousIntegerSources if n not in cv.unusedRegs +
            cv.branchRegisters]
        if suitableNums:
            num = random.choice(suitableNums)
            previousIntegerDestinations.append(num)
            while len(
                    previousIntegerDestinations) > \
                    numberOfPreviousRegistersToConsider:
...
def find_rand_source_reg():
    ...
    prob = random.uniform(0, 1)
    while len(previousIntegerSources) > \
            numberOfPreviousRegistersToConsider:
...
def find_rand_float_reg_destination():
    '''return a random writable FP register based on data hazard \
    probabilities'''
    ...
    while len(
            previousFloatDestinationRegisters) > \
            numberOfPreviousRegistersToConsider:
    ...
    # Write after read
    if previousFloatSourceRegisters and writeAfterWrite < prob < \
...
def find_rand_float_reg_source():
    ...
    while len(previousFloatSourceRegisters) > \
            numberOfPreviousRegistersToConsider:
