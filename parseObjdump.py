...
def parseObjectFile(
        fileName,
        bitwidth,
        codeFileName='code.hex',
        memoryFileName='memory.hex'):
        ...
        for line in inFile:
            if 'Contents of section' in line:
                if '.text' in line:
                    ...
                    if currentMemoryAddress > 0:
                        currentMemoryAddress = int(
                            lineSplits[0], 16) + (len(lineSplits) - 2) * 4
                else:
                    memoryStart = 2
                    memoryEndAddress = int(
                        lineSplits[0], 16) + (len(lineSplits) - 2) * 4
                    break
                continue
            ...
            if memoryStart == 1:
                ...
                if memoryFillZeros:
                    ...
                    if bitwidth == 64:
                        for cnt in range(
                                (currentLineAddress - currentMemoryAddress) // 8):
                            memoryOut.write('0000000000000000\n')
                    else:
                        for cnt in range(
                                (currentLineAddress - currentMemoryAddress) // 4):
    ...
    memoryOut.close()
    return {'memoryStartAddress': memoryStartAddress,
