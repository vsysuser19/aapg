...
def initialize_vars():
    ...
    loopPositions = [{'lValue': 0,
                      'targetInstr': 0,
                      'instrPosition': 0,
                      'setBranch': True,
                      'opcode': 'NULL',
                      'r1': 0,
                      'r2': 0} for indx in range(maxNestedLoops)]
    initializeLoopQueue = [{'r1': 0,
                            'r2': 0,
                            'initialized': True,
                            'instrPosition': 0,
                            'loopCount': 0} for indx in range(maxNestedLoops)]
...
def initializeLoop():
    ...
    for index, memb in enumerate(initializeLoopQueue):
        if memb['initialized'] == False and memb['instrPosition'] < \
            ...
            instr2 = 'addi x{0}, x0, {1}'.format(
                memb['r2'], randImmediate + memb['loopCount'])
...
def setBranchInstr():
    ...
    for indx, memb in enumerate(loopPositions):
        if memb['setBranch'] is False and memb['instrPosition'] <= \
                cv.instructionNumber:
            instr1 = 'addi x' + str(memb['r1']) + \
                ' , x' + str(memb['r1']) + ' , 1'
            if memb['opcode'] in ('bne', 'blt', 'bltu'):
                instr2 = '{0} x{1}, x{2}, i_{3}'.format(
                    memb['opcode'], memb['r1'], memb['r2'], memb['targetInstr'])
            else:
                instr2 = '{0} x{1}, x{2}, i_{3}'.format(
                    memb['opcode'], memb['r2'], memb['r1'], memb['targetInstr'])
...
def gen_base_instr(opcode):
    ...
    if opcode in op.base_i:
        ...
        if opcode in ("ld", "lb", "lh", "lw", "lbu", "lhu", 'lwu'):
            ...
            Instruction = "{0} x{1}, {2}(x{3})".format(
                opcode, j, l, memoryBaseAddressRegister)
        else:
        ...
        if opcode in ('srl', 'sra', 'sll', 'sllw', 'srlw', 'sraw'):
            ...
            if bitwidth == 64 and opcode not in ('sllw', 'srlw', 'sraw'):
                instructionTmp = 'addi x{0}, x0, {1}'.format(
                    l, random.randint(1, 63))
            else:
                instructionTmp = 'addi x{0}, x0, {1}'.format(
        ...
        if branchBackwardProbability > bType:
            ...
            for memb in loopPositions:
                if memb['instrPosition'] - \
                        2 < maxBranchPosition and memb['setBranch'] == False:
                    maxBranchPosition = memb['instrPosition'] - 2

            if maxBranchPosition < cv.instructionNumber + 6:
                return
            minBranchPosition = max(
                (max(previousBranches) + 4, cv.instructionNumber + 4))

            if minBranchPosition >= maxBranchPosition:
                return
            branchInstrPosition = random.randint(
                minBranchPosition, maxBranchPosition)
            ...
            for index, memb in enumerate(initializeLoopQueue):
                if memb['initialized']:
                    initializeLoopQueue[index] = {
                        'r1': j,
                        'r2': k,
                        'initialized': False,
                        'instrPosition': branchTarget - 2,
                        'loopCount': loopCount}
                    branchLock += 1
                    break
            ...
            for indx, member in enumerate(loopPositions):
                if member['setBranch']:
                    loopPositions[indx] = {
                        'lValue': branchInstrPosition - branchTarget,
                        'targetInstr': branchTarget,
                        'instrPosition': branchInstrPosition,
                        'r1': j,
                        'r2': k,
                        'opcode': opcode,
                        'setBranch': False}
            ...
            for branch in loopPositions:
                if branch['instrPosition'] in [
                    l + cv.instructionNumber + indx for indx in
                        range(-2, 2)] or branchLock > 0:
                    return ('nop',)

            previousBranches.append(l + cv.instructionNumber)
            previousBranches.popleft()
            Instruction = '{0} x{1}, x{2}, i_{3}'.format(
                opcode, j, k, l + cv.instructionNumber)
            return (Instruction,)
