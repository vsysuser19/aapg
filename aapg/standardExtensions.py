...
def gen_sp_fp_instr(opcode):
    ...
    if opcode in op.sp_fp_i + op.sp_fp_s:
        ...
        instruction = '{0} {1}, {2}(x{3})'.format(
            opcode, reg_f_1, immediate, memoryBaseAddressRegister)

    elif opcode in op.sp_fp_r4:
        # FMADD.S rd,rs1,rs2,rs3
        instruction = '{0} {1}, {2}, {3}, {4} {5}'.format(
            opcode, reg_f_1, reg_f_2, reg_f_3, reg_f_4, roundingmode)

    elif opcode in ('fadd.s', 'fsub.s', 'fmul.s', 'fdiv.s'):
        instruction = '{0} {1}, {2}, {3} {4}'.format(
            opcode, reg_f_1, reg_f_2, reg_f_3, roundingmode)

    elif opcode in ('fsgnj.s', 'fsgnjn.s', 'fsgnjx.s', 'fmin.s', 'fmax.s'):
        instruction = '{0} {1}, {2}, {3}'.format(
            opcode, reg_f_1, reg_f_2, reg_f_3)

    elif opcode in('fsqrt.s'):
        instruction = '{0} {1}, {2} {3}'.format(
            opcode, reg_f_1, reg_f_2, roundingmode)

    elif opcode in ('fcvt.w.s', 'fcvt.wu.s', 'fcvt.l.s', 'fcvt.lu.s'):
        reg_int = find_rand_destination_reg()
        instruction = '{0} x{1}, {2} {3}'.format(
            opcode, reg_int, reg_f_1, roundingmode)
        ...
        reg_int = find_rand_destination_reg()
        instruction = '{0} x{1}, {2}, {3}'.format(
            opcode, reg_int, reg_f_1, reg_f_2)

    elif opcode in ('fcvt.s.w', 'fcvt.s.wu', 'fcvt.s.l', 'fcvt.s.lu'):
        reg_int = find_rand_destination_reg()
        instruction = '{0} {1}, x{2} {3}'.format(
            opcode, reg_f_1, reg_int, roundingmode)
...
def gen_atomic_instr(opcode):
    ...
    if 'amo' in opcode:
        ...
        instr1 = 'addi x{0}, x{1}, {2}'.format(
            rs1, memoryBaseAddressRegister, rand_mem_location)
        ...
        try:
            ...
            return
        instr1 = 'addi x{0}, x{1}, {2}'.format(
            rs1, memoryBaseAddressRegister, mem_location)
        instr2 = '{0} x{1}, x{2}, (x{3})'.format(opcode, rd, rs2, rs1)
        return(instr1, instr2)
