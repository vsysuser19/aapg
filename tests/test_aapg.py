...
def test_all(runner,seed):
    ...
    result = runner.invoke(cli, ['gen','--num_programs=1','--static_make','--seed={}'.format(input_seed),'--setup_dir=tests/work/{config_file}'.format(config_file=list_of_files[7]),'--output_dir=tests/work/{config_file}/asm'.format(config_file=list_of_files[7])])
    try:
